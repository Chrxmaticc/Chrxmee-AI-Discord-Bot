...
module.exports = {
  ...
  async execute(interaction, client) {
      ...
      if (partyResult) {
        ...
        if (party.status === 'dungeon') {
          ...
          if (hp < 50) {
            ...
            await interaction.editReply(`🏃 You fled the party dungeon! **25% gold penalty** applied (HP was below 50).\n🪙 Gold left: **${data.gold}**`);
          } else {
    ...
    if (sub === 'party') {
      ...
      collector.on('collect', async btn => {
        ...
        if (btn.customId === `party_start_${userId}`) {
          ...
          if (currentParty.members.length < 2) return btn.followUp({ content: '❌ Need at least 2 players to start a party run!', ephemeral: true }).catch(() => {});
          collector.stop('starting');
          currentParty.status = 'readyup';
          client.memory.set(partyKey, currentParty);

          // Build ready-up embed
          const readyEmbed = new EmbedBuilder().setColor('#f0a500')
            .setTitle('⚔️ Ready Up!')
            .setDescription('All players must ready up to begin!')
            .addFields({
              name: '👥 Players',
              value: currentParty.members.map(m => `${m.userId === userId ? '👑' : '👤'} ${m.username} — ⏳ Waiting`).join('\n'),
              inline: false
            });
          ...
          readyCollector.on('collect', async rbtn => {
            ...
            if (rbtn.customId === `party_ready_${userId}`) {
              readyStatus[rbtn.user.id] = true;
            } else if (rbtn.customId === `party_notready_${userId}`) {
              readyStatus[rbtn.user.id] = false;
            }

            // Update embed
            const updatedReadyEmbed = new EmbedBuilder().setColor('#f0a500')
              .setTitle('⚔️ Ready Up!')
              .setDescription('All players must ready up to begin!')
              .addFields({
                name: '👥 Players',
                value: rParty.members.map(m => `${m.userId === userId ? '👑' : '👤'} ${m.username} — ${readyStatus[m.userId] === true ? '✅ Ready' : readyStatus[m.userId] === false ? '❌ Not Ready' : '⏳ Waiting'}`).join('\n'),
                inline: false
              });
            ...
            if (allReady) {
              ...
              rParty.status = 'dungeon';
              rParty.currentRoom = 1;
              for (const m of rParty.members) {
                rParty.memberHp[m.userId] = 100;
                rParty.memberAlive[m.userId] = true;
              }
      ...
      collector.on('end', (_, reason) => {
        if (reason !== 'starting' && reason !== 'disbanded') {
          lobbyMsg.edit({ components: [] }).catch(() => {});
        }
      });
      return;
    ...
    if (sub === 'invite') {
      ...
      if (party.members.length >= 4) return interaction.editReply('❌ Party is full! (4/4 players)');
      ...
      if (party.members.some(m => m.userId === target.id)) return interaction.editReply('❌ That player is already in your party!');

      const inviteEmbed = new EmbedBuilder().setColor('#9b59b6')
        .setTitle('⚔️ Party Invitation!')
        .setDescription(`**${username}** is inviting you to join their dungeon party!\n\nYou have 60 seconds to accept or decline.`)
        .addFields({
          name: '👥 Current Members',
          value: party.members.map(m => `${m.userId === userId ? '👑' : '👤'} ${m.username}`).join('\n'),
          inline: false
        });
      ...
      collector.on('collect', async btn => {
        ...
        const currentParty = client.memory.get(partyKey);
        if (!currentParty) return btn.followUp({ content: '❌ The party no longer exists!', ephemeral: true }).catch(() => {});

        if (btn.customId === `invite_accept_${userId}_${target.id}`) {
          if (currentParty.members.length >= 4) {
            return btn.followUp({ content: '❌ Party is now full!', ephemeral: true }).catch(() => {});
          }
          currentParty.members.push({ userId: target.id, username: target.username });
          currentParty.memberHp[target.id] = 100;
          currentParty.memberAlive[target.id] = true;
          client.memory.set(partyKey, currentParty);
          collector.stop('accepted');

          await inviteMsg.edit({
            embeds: [new EmbedBuilder().setColor('#4caf50')
              .setTitle('✅ Invite Accepted!')
              .setDescription(`**${target.username}** joined the party!`)
              .addFields({
                name: `👥 Members (${currentParty.members.length}/4)`,
                value: currentParty.members.map(m => `${m.userId === userId ? '👑' : '👤'} ${m.username}`).join('\n'),
                inline: false
              })],
            components: []
          }).catch(() => {});
          ...
          await inviteMsg.edit({
            embeds: [new EmbedBuilder().setColor('#ff0000').setTitle('❌ Invite Declined').setDescription(`**${target.username}** declined the party invite.`)],
            components: []
          }).catch(() => {});
        }
      });
    ...
    if (sub === 'start') {
      ...
      const embed = new EmbedBuilder().setColor('#2f3136')
        .setTitle('⚔️ Choose Your Mode')
        .setDescription('How do you want to enter the dungeon?')
        .addFields(
          { name: '🗡️ Solo', value: 'Enter alone. Your own pace, your own glory.', inline: true },
          { name: '⚔️ Party', value: inParty ? `You\'re in a party with ${partyResult.party.members.length} players! Ready to go.` : '❌ You need to be in a party first.\nUse `/dungeon party` to create one.', inline: true }
        );
      ...
      collector.on('collect', async btn => {
        ...
        if (btn.customId === 'mode_solo') {
          ...
          if (pr.party.hostId !== userId) return btn.followUp({ content: '❌ Only the party host can start the run!', ephemeral: true }).catch(() => {});
          const { key: partyKey, party } = pr;
          party.status = 'dungeon';
          party.currentRoom = 1;
          for (const m of party.members) {
            party.memberHp[m.userId] = 100;
            party.memberAlive[m.userId] = true;
          }
          client.memory.set(partyKey, party);
      ...
      collector.on('end', collected => {
        if (collected.size === 0) modeMsg.edit({ components: [] }).catch(() => {});
      });
    }
  }
};
...
async function performBossRoom(msg, client, data, guildId, userId, username) {
  ...
  collector.on('collect', async btn => {
    ...
    const totalAttackBonus = swordBonus + (activePets.includes('wolf') ? 10 : 0) + (activePets.includes('dragon') ? 8 : 0);
    ...
    if (action.startsWith('spell_')) {
      ...
      data.damageBoostActive = false;
      if (action === 'b_strike') {
        if (bossAction === 'punch') { playerHpLoss = Math.max(0, 20 - armorBonus); bossHpLoss = (25 + totalAttackBonus) * boost; roundLog = `Struck! Boss punched. **-${playerHpLoss} HP** / **-${bossHpLoss} Boss HP**`; }
        else { playerHpLoss = Math.max(0, 10 - armorBonus); roundLog = `Struck but boss parried! **-${playerHpLoss} HP**`; }
      } else if (action === 'b_defend') {
        if (bossAction === 'punch') { playerHpLoss = Math.max(0, Math.floor((20 - armorBonus) / 2)); roundLog = `Defended! **-${playerHpLoss} HP**`; }
        else { roundLog = `Both defending... stalemate.`; }
      } else if (action === 'b_explosive') {
        if (bossAction === 'punch') { playerHpLoss = Math.max(0, 30 - armorBonus); bossHpLoss = (45 + totalAttackBonus) * boost; roundLog = `BOOM! **-${playerHpLoss} HP** / **-${bossHpLoss} Boss HP**`; }
        else { playerHpLoss = Math.max(0, 15 - armorBonus); bossHpLoss = (25 + totalAttackBonus) * boost; roundLog = `Explosion! **-${playerHpLoss} HP** / **-${bossHpLoss} Boss HP**`; }
      } else if (action === 'b_parry') {
        if (bossAction === 'punch') { bossHpLoss = (30 + totalAttackBonus) * boost; roundLog = `🔰 PERFECT PARRY! **-${bossHpLoss} Boss HP**`; }
    ...
    if (data.hp <= 0) {
      ...
      await msg.edit({ embeds: [new EmbedBuilder().setColor('#ff0000').setTitle(`💀 ${username} slain! Reviving in 4s...`).addFields({ name: 'HP Bar', value: getDieBar(0), inline: false })], components: [disabledRow] }).catch(() => {});
...
async function performRoom(msg, client, data, guildId, userId, username) {
  ...
  collector.on('collect', async btn => {
    ...
    if (data.hp <= 0) {
      ...
      data.currentRoom++;
      await btn.followUp({ embeds: [new EmbedBuilder().setColor('#f0a500').setTitle('Room Result').setDescription(result).addFields({ name: 'HP Bar', value: getDieBar(data.hp) }, { name: '❤️ HP', value: `${data.hp}`, inline: true }, { name: '🪙 Gold', value: `${data.gold}`, inline: true })], ephemeral: true }).catch(() => {});
    }
    client.memory.set(`dungeon_${guildId}_${userId}`, data);
    await saveStats(client.pool, userId, guildId, username, data.farthestRoom, data.totalGoldEarned);
...
async function performPartyRoom(msg, client, partyKey, guildId) {
  ...
  if (aliveMembers.length === 0) {
    await msg.edit({ embeds: [new EmbedBuilder().setColor('#ff0000').setTitle('💀 Party Wiped!').setDescription('All members have fallen. The dungeon claims another party...')], components: [] }).catch(() => {});
    client.memory.delete(partyKey);
    return;
  }
  ...
  const roomTheme = ROOM_THEMES[Math.floor(Math.random() * ROOM_THEMES.length)];
  const memberLines = party.members.map(m => {
    const hp = party.memberHp[m.userId] || 0;
    const alive = party.memberAlive[m.userId];
    return `${alive ? '❤️' : '💀'} **${m.username}**: ${alive ? `${hp}/100 HP` : 'Dead'}`;
  }).join('\n');

  const embed = new EmbedBuilder().setColor('#9b59b6')
    .setTitle(`⚔️ Party — Room ${party.currentRoom} — ${roomTheme}`)
    .setDescription('Vote on your action! Majority wins. Tie = random.')
    .addFields({ name: '👥 Party Status', value: memberLines, inline: false });
  ...
  const votes = {};
  const voted = new Set();
  const voteTotals = { fight: 0, sneak: 0, loot: 0, surrender: 0 };
  ...
  // Tally votes
  let winningAction = 'fight';
  let maxVotes = 0;
  for (const [action, count] of Object.entries(voteTotals)) {
    if (count > maxVotes) { maxVotes = count; winningAction = action; }
  }
  // Tie = random
  ...
  // Handle surrender (majority required = more than half)
  if (winningAction === 'surrender') {
    const surrenderVotes = voteTotals.surrender;
    if (surrenderVotes <= totalAlive / 2) {
      winningAction = 'fight'; // not majority
    } else {
      // Party surrenders
      for (const m of party.members) {
        const dData = client.memory.get(`dungeon_${guildId}_${m.userId}`);
        if (dData) { dData.inDungeon = false; client.memory.set(`dungeon_${guildId}_${m.userId}`, dData); }
      }
      client.memory.delete(partyKey);
  ...
  // Apply action to all alive members
  let goldGain = 0, hpLossEach = 0, resultDesc = '';
  if (winningAction === 'fight') {
    goldGain = 40; hpLossEach = 15;
    resultDesc = `⚔️ Party fought! Each member earned **+${goldGain}g** and took **-${hpLossEach} HP**.`;
  } else if (winningAction === 'sneak') {
    goldGain = 25;
    if (Math.random() < 0.25) { hpLossEach = 3; resultDesc = `🥷 Sneaked but got grazed! **+${goldGain}g** / **-${hpLossEach} HP** each.`; }
    else { resultDesc = `🥷 Sneaked safely! **+${goldGain}g** each.`; }
  } else if (winningAction === 'loot') {
    goldGain = 60;
    if (Math.random() < 0.25) { hpLossEach = 3; resultDesc = `💰 Looted but triggered a trap! **+${goldGain}g** / **-${hpLossEach} HP** each.`; }
    else { resultDesc = `💰 Looted clean! **+${goldGain}g** each.`; }
  }
  ...
  for (const m of party.members) {
    ...
    if (party.memberHp[m.userId] <= 0) {
      deaths.push(m.username);
      party.memberAlive[m.userId] = false;
      party.memberHp[m.userId] = 50;
      dData.gold = Math.floor(dData.gold * 0.5);
    }
  ...
  const updatedMemberLines = party.members.map(m => {
    const hp = party.memberHp[m.userId] || 0;
    const alive = party.memberAlive[m.userId];
    return `${alive ? '❤️' : '💀'} **${m.username}**: ${alive ? `${hp}/100 HP` : 'Dead (revived next room)'}`;
  }).join('\n');

  await msg.edit({ embeds: [new EmbedBuilder().setColor('#9b59b6')
    .setTitle(`⚔️ Room ${party.currentRoom - 1} Result`)
    .setDescription(resultDesc)
    .addFields({ name: '👥 Party Status', value: updatedMemberLines, inline: false })
    .setFooter({ text: 'Next room in 3 seconds...' })], components: [] }).catch(() => {});
...
async function performPartyBossRoom(msg, client, partyKey, guildId) {
  ...
  const embed = new EmbedBuilder().setColor('#ff4444')
    .setTitle(`⚠️ PARTY BOSS — ${boss.emoji} ${boss.name}`)
    .setDescription(`*${boss.flavor}*\n\nVote on your action! Majority wins.`)
    .addFields(
      { name: `${boss.emoji} Boss HP`, value: `${getBossBar(boss.currentHp, boss.maxHp)} ${boss.currentHp}/${boss.maxHp}`, inline: false },
      { name: '👥 Party Status', value: memberLines, inline: false }
    );
  ...
  const totalAlive = aliveMembers.length;
  const voted = new Set();
  const voteTotals = { strike: 0, defend: 0, explosive: 0, parry: 0, flee: 0 };
  ...
  if (winAction === 'flee') {
    const goldLostEach = 100;
    for (const m of aliveMembers) {
      const dData = client.memory.get(`dungeon_${guildId}_${m.userId}`) || getDefaultData(m.userId);
      dData.gold = Math.max(0, (dData.gold || 0) - goldLostEach);
      client.memory.set(`dungeon_${guildId}_${m.userId}`, dData);
    }
    party.currentRoom++;
    client.memory.set(partyKey, party);
    await msg.edit({ embeds: [new EmbedBuilder().setColor('#888888').setTitle('🏃 Party Fled!').setDescription(`Each member lost **${goldLostEach}g**!`)], components: [] }).catch(() => {});
    setTimeout(() => performPartyRoom(msg, client, partyKey, guildId), 2000);
    return;
  }

  // Calculate combined party attack (average armor/sword)
  let totalArmorBonus = 0, totalSwordBonus = 0;
  for (const m of aliveMembers) {
    const dData = client.memory.get(`dungeon_${guildId}_${m.userId}`);
    if (dData) {
      const armor = ARMOR_SHOP.find(a => a.id === dData.equippedArmor);
      const sword = SWORDS.find(s => s.id === dData.equippedSword);
      if (armor) totalArmorBonus += armor.hpBonus;
      if (sword) totalSwordBonus += sword.bonusDmg;
    }
  }
  const avgArmor = Math.floor(totalArmorBonus / totalAlive);
  const avgSword = Math.floor(totalSwordBonus / totalAlive);
  const bossAction = Math.random() < 0.70 ? 'punch' : 'parry';

  let bossHpLoss = 0, hpLossEach = 0, roundLog = '';

  if (winAction === 'strike') {
    if (bossAction === 'punch') { hpLossEach = Math.max(0, 20 - avgArmor); bossHpLoss = 25 + avgSword; roundLog = `Party struck! Boss punched ALL members. **-${hpLossEach} HP each** / **-${bossHpLoss} Boss HP**`; }
    else { hpLossEach = Math.max(0, 10 - avgArmor); roundLog = `Party struck but boss parried! **-${hpLossEach} HP each**`; }
  } else if (winAction === 'defend') {
    if (bossAction === 'punch') { hpLossEach = Math.max(0, Math.floor((20 - avgArmor) / 2)); roundLog = `Party defended! **-${hpLossEach} HP each**`; }
    else { roundLog = `Party defended and boss parried... stalemate!`; }
  } else if (winAction === 'explosive') {
    if (bossAction === 'punch') { hpLossEach = Math.max(0, 30 - avgArmor); bossHpLoss = 45 + avgSword; roundLog = `BOOM! Boss punched through. **-${hpLossEach} HP each** / **-${bossHpLoss} Boss HP**`; }
    else { hpLossEach = Math.max(0, 15 - avgArmor); bossHpLoss = 25 + avgSword; roundLog = `Explosion! Boss half-blocked. **-${hpLossEach} HP each** / **-${bossHpLoss} Boss HP**`; }
  } else if (winAction === 'parry') {
    if (bossAction === 'punch') { bossHpLoss = 30 + avgSword; roundLog = `🔰 PARTY PERFECT PARRY! **-${bossHpLoss} Boss HP**`; }
    else { hpLossEach = Math.max(0, 8 - avgArmor); bossHpLoss = 8; roundLog = `⚡ PARRY CLASH! **-${hpLossEach} HP each** / **-${bossHpLoss} Boss HP**`; }
  }

  boss.currentHp = Math.max(0, boss.currentHp - bossHpLoss);

  // Apply damage to all alive members
  const deaths = [];
  for (const m of aliveMembers) {
    party.memberHp[m.userId] = Math.max(0, (party.memberHp[m.userId] || 100) - hpLossEach);
    const dData = client.memory.get(`dungeon_${guildId}_${m.userId}`) || getDefaultData(m.userId);
    if (party.memberHp[m.userId] <= 0) {
      ...
      party.memberAlive[m.userId] = false;
      party.memberHp[m.userId] = 50;
      dData.gold = Math.floor((dData.gold || 0) * 0.5);
    }
    client.memory.set(`dungeon_${guildId}_${m.userId}`, dData);
  }
  ...
  // Boss defeated
  if (boss.currentHp <= 0) {
    const goldPerMember = Math.floor(450 / stillAlive.length);
    let spellDrop = null;
    if (Math.random() < 0.4) {
      const rs = SPELLS[Math.floor(Math.random() * SPELLS.length)];
      const luckyMember = stillAlive[Math.floor(Math.random() * stillAlive.length)];
      const lData = client.memory.get(`dungeon_${guildId}_${luckyMember.userId}`) || getDefaultData(luckyMember.userId);
      lData.spells[rs.id] = (lData.spells[rs.id] || 0) + 1;
      client.memory.set(`dungeon_${guildId}_${luckyMember.userId}`, lData);
      spellDrop = { spell: rs, member: luckyMember.username };
    }

    for (const m of stillAlive) {
      const dData = client.memory.get(`dungeon_${guildId}_${m.userId}`) || getDefaultData(m.userId);
      dData.gold = (dData.gold || 0) + goldPerMember;
      if (dData.gold > (dData.totalGoldEarned || 0)) dData.totalGoldEarned = dData.gold;
      client.memory.set(`dungeon_${guildId}_${m.userId}`, dData);
    }
    ...
    await msg.edit({ embeds: [new EmbedBuilder().setColor('#FFD700')
      .setTitle(`🏆 ${boss.emoji} ${boss.name} DEFEATED by the Party!`)
  ...
  // All dead
  if (stillAlive.length === 0) {
    await msg.edit({ embeds: [new EmbedBuilder().setColor('#ff0000').setTitle(`💀 Party Wiped by ${boss.emoji} ${boss.name}!`).setDescription('All members have fallen...')], components: [] }).catch(() => {});
    client.memory.delete(partyKey);
    return;
  }

  // Update party and continue boss fight
  const updatedMemberLines = party.members.map(m => {
    const hp = party.memberHp[m.userId] || 0;
    const alive = party.memberAlive[m.userId];
    return `${alive ? '❤️' : '💀'} **${m.username}**: ${alive ? `${hp}/100 HP` : 'Dead'}`;
  ...
  const updatedEmbed = new EmbedBuilder().setColor('#ff4444')
    .setTitle(`⚠️ PARTY BOSS — ${boss.emoji} ${boss.name}`)
    .setDescription(`**Last round:** ${roundLog}\n\nVote on your next action!`)
    .addFields(
      { name: `${boss.emoji} Boss HP`, value: `${getBossBar(boss.currentHp, boss.maxHp)} ${boss.currentHp}/${boss.maxHp}`, inline: false },
      { name: '👥 Party Status', value: updatedMemberLines, inline: false }
    );
