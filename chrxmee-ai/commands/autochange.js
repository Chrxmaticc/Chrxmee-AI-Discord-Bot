...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (sub === "setup") {
      ...
      await pool.query(
        `INSERT INTO server_autochange (guild_id, interval_hours) VALUES ($1, $2)
      ...
      const embed = new EmbedBuilder()
        .setColor(0x7c7ce0)
        .setTitle(`${E.settings} Server Autochange Interval Set`)
    ...
    // ─── ASSET MANAGEMENT ──────────────────────
    async function manageAsset(type, valueGetter, optionName) {
    ...
    if (sub === "name" || sub === "icon" || sub === "banner" || sub === "description") {
      const type = sub === "name" ? "names" : sub === "icon" ? "icons" : sub === "banner" ? "banners" : "descriptions";
      const option = interaction.options.getString("text") || interaction.options.getString("url");
      return manageAsset(type, () => option, optionName);
    }

    // ─── CHANNEL RENAME (special handling) ────
