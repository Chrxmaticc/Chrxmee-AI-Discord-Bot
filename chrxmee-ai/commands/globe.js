    ...
    try {
      ...
      for (let i = 0; i < frames; i++) {
        ...
        for (let lon = 0; lon < 6; lon++) {
          ...
          ctx.beginPath();
          ctx.ellipse(centerX, centerY, radius, radius * 0.35, lonAngle, 0, Math.PI * 2);
        ...
        for (let lat = 0; lat < 4; lat++) {
          ...
          ctx.beginPath();
          ctx.ellipse(centerX, centerY + latOffset, latRadius, 4, 0, 0, Math.PI * 2);
          ctx.strokeStyle = "rgba(255,255,255,0.1)";
          ctx.lineWidth = 1;
          ctx.stroke();
        }

        // Avatar mapping (view-dependent)
        ...
        if (sides === "all" || (sides === "front" && facingCamera)) {
          ...
          // Map avatar to globe surface with perspective
          const scaleX = Math.cos(angle);
          const avatarWidth = radius * 2 * Math.abs(scaleX);
          const avatarX = centerX - avatarWidth / 2;

          if (avatarWidth > 5) {
            const opacity = sides === "all" ? 1 : Math.max(0, scaleX);
