  ...
  async execute(interaction, client) {
    ...
    if (sub === 'give') {
      ...
      if (!isOwnerGive && amount > MOD_CAP) {
        return interaction.editReply(`❌ Moderators can only give up to **${MOD_CAP} dungeon gold** at a time. Ask the bot owner for more.`);
      }
      const tKey = `dungeon_${guildId}_${target.id}`;
      let tData = client.memory.get(tKey) || getDefaultDungeonData();
      tData = migrateDungeonData(tData);
      tData.gold += amount;
          ...
          if (logChannel) {
            ...
            await logChannel.send({ embeds: [new LE()
              ...
              .addFields(
                ...
                { name: '💰 Amount',   value: `${amount} dungeon gold`, inline: true },
      ...
      return interaction.editReply(`✅ Gave **${amount} dungeon gold** to **${target.username}**! They now have **${tData.gold}**.`);
    }
    // ── RESET ──────────────────────────────────────────────
    if (sub === 'reset') {
